...
def total_variance_loss(image):#노이즈 있는 픽셀값 방지하기위해서 씀 이미지 부드러워짐
...
def fit(X, content, style, args):
    style_weights = [4.0, 3.0, 1.5, 1.0, 0.5]#가중치 주어서 비슷하게 만들기 얕은레이어에 강한가중치 주고 깊은 레이어에 작은 가중치 
...
def main(args):
    ...
    X = Variable(content.clone(), requires_grad=True) #true = 학습해야할 parameter
